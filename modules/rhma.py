...
class Rhma:
    ...
    def schedule(self, busy_period_list, number_of_cores, assignment, hyperperiod):
        busy_period_list = busy_period_list[0:5] #TODO enlever to do
        ...
        for i in range(len(s_i_h_param)):
            ...
            for h in range(len(s_i_h_param[i])):
                ...
                for a in s_i_h_param[i][h]:
                    temp_h.append([a*taskset[i].period, (a+1)*taskset[i].period])
                temp.append(temp_h)
            r_i_h_a_param.append(temp)
        ...
        for h, busy_period in enumerate(busy_period_list): 
            ...
            for i in range(len(s_i_h_param)):
                ...
                for a_index, a in enumerate(s_i_h_param[i][h]):
                    ...
                    for j in range(number_of_cores):
                        x[i][a][j] = {}
                        for t in range(int(r_i_h_a_param[i][h][a_index][0]), int(r_i_h_a_param[i][h][a_index][1])):
                ...
                for a_index, a in enumerate(s_i_h_param[i][h]):
                    for k in range(len(s_i_h_param)):
                        for b_index, b in enumerate(s_i_h_param[k][h]):
                            if k > i and not self.has_intersection(r_i_h_a_param[i][h][a_index], r_i_h_a_param[k][h][b_index]):
                                prob += m[i][a][k][b] == 0  
            ...
            for i in range(len(s_i_h_param)):
                for j in range(number_of_cores):
                    left = lpSum(x[i][a][j][t] for a_index, a in enumerate(s_i_h_param[i][h]) for t in range(int(r_i_h_a_param[i][h][a_index][0]), int(r_i_h_a_param[i][h][a_index][1])))
            ...
            for i in range(len(s_i_h_param)):
                for j in range(number_of_cores):
                    for a_index, a in enumerate(s_i_h_param[i][h]):
                        left = lpSum(x[i][a][j][t] for t in range(int(r_i_h_a_param[i][h][a_index][0]), int(r_i_h_a_param[i][h][a_index][1])))
