...
class InterferenceGenerator:
    ...
    def generate_interference(self, task_wcets):
        interferences = numpy.zeros((self.taskset_count, self.tasks_per_taskset, self.tasks_per_taskset))
        for taskset_index in range(self.taskset_count):
            interferences[taskset_index] = self.generate_taskset_interference(task_wcets[taskset_index])
        return interferences

    def generate_taskset_interference(self, task_wcets):
        taskset_interference_matrix = numpy.zeros((self.tasks_per_taskset, self.tasks_per_taskset))
        for task_index_i in range(self.tasks_per_taskset):
            for task_index_j in range(task_index_i + 1, self.tasks_per_taskset):
                ...
                taskset_interference_matrix[task_index_i][task_index_j] = interference_occurs * math.floor(
                    self.interference_factor * 0.5 * min(task_wcets[task_index_i], task_wcets[task_index_j])
                )
        ...
        return taskset_interference_matrix
