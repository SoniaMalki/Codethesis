...
class PrimeMatrixLoaderSaver:
    ...
    def load(self, max_hyperperiod, max_prime, gen_limit_exponent, index=0):
        matrix_filename = self.get_matrix_filename(
            max_hyperperiod, max_prime, gen_limit_exponent, index)
        with open(self.matrix_path / matrix_filename, 'rb') as f:
            prime_matrix = pickle.load(f)
        return prime_matrix

    def save(self, prime_matrix, max_hyperperiod, max_prime, gen_limit_exponent, index=0):
