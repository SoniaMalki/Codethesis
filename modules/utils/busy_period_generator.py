...
class BusyPeriodGenerator:
    ...
    def generate_busy_periods_from_schedule(scheduling, busy_period_timeslices):
        ...
        for start, end in busy_period_timeslices:
            ...
            for core_schedule in scheduling:
                core_busy_period_solo = [
                    item for item in core_schedule if start <= item[0] <= end]
                if core_busy_period_solo:
                    core_busy_periods.append(core_busy_period_solo)

            # Calculer total_utilization pour ce Scheduling
            total_utilization = 0
            for core_schedule in core_busy_periods:
                for _, task, job in core_schedule:
                    if task is not None and job is not None:
                        total_utilization += 1

            core_schedule_obj = Scheduling(
                schedule=core_busy_periods, success=scheduling.success, scheduler_name=scheduling.scheduler_name)
            core_schedule_obj.add_total_utilization(
    ...
    def generate_busy_periods(scheduling):
        ...
        busy_period_timeslices = BusyPeriodGenerator.generate_busy_periods_timeslice(
            monocore_busy_periods_timeslices)
        busy_period = BusyPeriodGenerator.generate_busy_periods_from_schedule(
            scheduling=scheduling, busy_period_timeslices=busy_period_timeslices)
        return busy_period
