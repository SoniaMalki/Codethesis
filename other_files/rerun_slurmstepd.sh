...
if [ $# -lt 3 ]; then
  echo "Usage: $0 <suffixe (nic5, lemaitre4, hercules, dragon2)> <experience_number> <error_type (TimeLimit, NodeFailure, etc.)>"
  exit 1
fi
...
if [ -z "$SUFFIXE" ]; then
  ERROR_DIR="$base_dir/error_handling/slurm_error/$ERROR_TYPE"
  SBATCH_DIR="$GENERATION_DIR/$EXPERIENCE/slurm/slurm_files"
else
  ERROR_DIR="$base_dir/error_handling/slurm_error_${SUFFIXE}/$ERROR_TYPE"
